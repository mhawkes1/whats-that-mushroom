...
class DescribeRequest(BaseModel):
    description: str = Field(
        max_length=2000,
        description="A sentence or two, in the user's own words.",
    )
...
class DescribeOut(BaseModel):
    field_notes: dict[str, str] = Field(
        description=(
            "Character key -> answer option, ready for /identify. A suggestion "
            "to pre-fill the form with, never an auto-submission: the user "
            "confirms it, exactly as they do a matched spore print colour."
        )
    )
    not_understood: list[str] = Field(
        description=(
            "Phrases the translator could not place. Shown to the user, so "
            "the gap is theirs to close rather than silently dropped."
        )
    )
    rejected: dict[str, str] = Field(
        default_factory=dict,
        description=(
            "Values that are not catalogue options, dropped. Should always be "
            "empty; a non-empty one means a guarantee did not hold."
        ),
    )
    backend: str = Field(
        description=(
            "Which translator ran -- a model id, or 'keywords' for the "
            "offline stand-in. Never hidden, in the same spirit as /health "
            "reporting model_loaded: false."
        )
    )
